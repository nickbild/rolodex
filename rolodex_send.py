...
# April 2025
#
# This script encodes a string of characters into a format that can be sent to the Rolodex,
# then sends it over a serial connection.
#
# Usage:
# python3 rolodex_send.py "I AM SEDNDING A TEST TO THE ROLODEX."
####
...
input_len = math.floor(len(input_data) / 4)
leftover = len(input_data) % 4
output = []

# Process the input in blocks of 4 characters.
for i in range(input_len):
    block = input_data[i * 4:i * 4 + 4]
...
if leftover > 0:
    for i in range(leftover):
        block = input_data[input_len * 4 + i]
